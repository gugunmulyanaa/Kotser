...
class Setting extends PoCore
{
						...
						<div role="tabpanel" class="tab-pane" id="image">
							<div class="table-responsive">
								<table class="table table-striped table-hover table-setting">
									...
									<tr>
										<td style="width:265px;"><?= $GLOBALS['_']['setting_photo']; ?></td>
										<td class="link-setting">
											<?php if (file_exists('../' . DIR_INC . '/images/walikota.jpg')) { ?>
												<img src="../<?= DIR_INC; ?>/images/walikota.jpg" class="thumbnail img-responsive" width="200" />
											<?php } ?>
											<?= $this->pohtml->formStart(array('method' => 'post', 'action' => 'route.php?mod=setting&act=editphoto', 'enctype' => true, 'autocomplete' => 'off')); ?>
											<div class="input-group col-md-5">
												<input class="form-control input-sm" type="file" name="picture" />
												<span class="input-group-btn">
													<button class="btn btn-sm btn-primary" type="submit"><i class="fa fa-check"></i> <?= $GLOBALS['_']['action_5']; ?></button>
	...
	public function editphoto()
	{
		...
		if (!empty($_FILES)) {
			if (!empty($_FILES['picture']['tmp_name'])) {
				if (file_exists('../' . DIR_INC . '/images/walikota.jpg')) {
					unlink('../' . DIR_INC . '/images/walikota.jpg');
				}
				$upload = new PoUpload($_FILES['picture']);
				if ($upload->uploaded) {
					$upload->file_new_name_body = 'walikota';
